    ...
    public class SvgGdi : IGdi
	{
		...
		public void ExtTextOut(short x, short y, ushort options, short[] rect, byte[] text, short[] dx)
		{
			...
			if (vertical)
			{
				...
				if (dc.Font != null) width = Math.Abs(dc.Font.FontSize);
			}
			else
			{
				if (dc.Font != null)
				...
				{
					if (dc.Font != null) width = (short)(Math.Abs(dc.Font.FontSize * text.Length) / 2);
					elem.Add(new XAttribute("x", ax));
				}
			}
			...
			if (vertical)
			{
				...
				{
					if (dc.Font != null) height = (short)(Math.Abs(dc.Font.FontSize * text.Length) / 2);
				}
				elem.Add(new XAttribute("y", sb.ToString()));
			}
			else
			{
				if (dc.Font != null) height = Math.Abs(dc.Font.FontSize);
				if (compatible)
				{
		...
		public void FillRgn(IGdiRegion rgn, IGdiBrush brush)
		{
			...
			XElement elem = new XElement(svg + "use",
				...
				new XAttribute("class", GetClassString(brush)));
		...
		public void LineTo(short ex, short ey)
		{
			XElement elem = new XElement(svg + "line",
				dc.Pen != null ? new XAttribute("class", GetClassString(dc.Pen)) : null,
